...
<div id="exampleModal" class="fixed inset-0 z-[999] hidden items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="exampleModalLongTitle" tabindex="-1" data-keyboard="false" data-backdrop="static">
    ...
    <div class="relative w-full max-w-7xl h-[90vh] flex flex-col rounded-2xl border border-dark_border bg-dark_grey shadow-2xl overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-dark_border">
            <h5 class="text-lg font-semibold text-white flex flex-wrap items-baseline gap-x-2 gap-y-1" id="exampleModalLongTitle">
                <span>File content</span>
                <span id="csvPreviewRowCount" class="text-sm font-normal text-muted tabular-nums" aria-live="polite"></span>
            </h5>
        ...
        <div class="flex-1 overflow-auto px-6 py-4 flex flex-col min-h-0">
            <div class="table-responsive table-modal overflow-x-auto text-white text-sm flex-1 min-h-0"></div>
            <div id="bulk-preview-pagination" class="hidden shrink-0 mt-4 pt-4 border-t border-dark_border flex flex-wrap items-center justify-between gap-3">
                <button type="button" id="bulk-preview-prev" disabled
                        class="px-4 py-2 rounded-lg border border-dark_border text-sm font-medium text-grey hover:text-white hover:bg-white/10 transition disabled:opacity-40 disabled:pointer-events-none">
                    Previous
                </button>
                <span id="bulk-preview-page-label" class="text-sm text-muted text-center flex-1 min-w-[12rem]"></span>
                <button type="button" id="bulk-preview-next" disabled
                        class="px-4 py-2 rounded-lg border border-dark_border text-sm font-medium text-grey hover:text-white hover:bg-white/10 transition disabled:opacity-40 disabled:pointer-events-none">
                    Next
                </button>
            </div>
...
{{-- Payment modal: existing Planet F user (email + PIN) or new user (email only) --}}
<div id="paymentModal" class="fixed inset-0 z-[999] hidden items-center justify-center p-4" role="dialog" aria-modal="true" tabindex="-1" data-keyboard="false" data-backdrop="static">
    <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" data-dismiss="backdrop"></div>
    <div class="relative w-full max-w-md rounded-2xl border border-dark_border bg-dark_grey shadow-2xl px-6 py-8">
        ...
        <div class="flex gap-2 mb-6" role="radiogroup" aria-label="PlanetF account">
            <label class="flex-1 cursor-pointer account-choice-label">
                <input type="radio" name="planetf_account" value="yes" class="sr-only" id="account-yes" autocomplete="off">
                <span class="account-choice-card block rounded-lg border border-dark_border bg-darkmode/40 py-3 text-center text-sm text-grey transition hover:border-primary/50 hover:bg-primary/5 hover:text-white">
                    Yes
                </span>
            </label>
            <label class="flex-1 cursor-pointer account-choice-label">
                <input type="radio" name="planetf_account" value="no" class="sr-only" id="account-no" checked autocomplete="off">
                <span class="account-choice-card block rounded-lg border border-dark_border bg-darkmode/40 py-3 text-center text-sm text-grey transition hover:border-primary/50 hover:bg-primary/5 hover:text-white">
                    No
                </span>
            </label>
...
<div id="updateTableModal" class="fixed inset-0 z-[999] hidden items-center justify-center p-4" role="dialog" aria-modal="true" tabindex="-1" data-keyboard="false" data-backdrop="static">
    ...
    <div class="relative w-full max-w-7xl h-[90vh] flex flex-col rounded-2xl border border-dark_border bg-dark_grey shadow-2xl overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-dark_border">
            <h5 class="text-lg font-semibold text-white flex flex-wrap items-baseline gap-x-2 gap-y-1">
                <span>File content</span>
                <span id="updateTableRowCount" class="text-sm font-normal text-muted tabular-nums" aria-live="polite"></span>
            </h5>
            <button type="button" class="p-2 rounded-lg text-grey hover:text-white hover:bg-white/10 transition" data-dismiss="modal" aria-label="Close">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
